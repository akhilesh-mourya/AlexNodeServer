...
Chat.getAll = (req, result) => {
  ...
  if (req?.query?.nextCursor > 0) {
    ...
    FROM messages LEFT JOIN user ON messages.user_id = user.id where messages._id < ${req?.query?.nextCursor} ORDER BY  messages.createdAt DESC, messages._id LIMIT 10`;
  } else {
    query = `SELECT messages._id, messages.text, messages.user_id, messages.createdAt, user.name, user.avatar 
    FROM messages LEFT JOIN user ON messages.user_id = user.id ORDER BY messages.createdAt DESC, messages._id LIMIT 10`;
  }
